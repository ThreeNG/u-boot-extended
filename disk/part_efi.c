...
static char *print_efiname(gpt_entry *pte)
{
	...
	for (i = 0; i < PARTNAME_SZ; i++) {
		u8 c;
		//@  assert \initialized(&pte->partition_name[i]);
...
{
  //@ assert \initialized(&p->attributes.fields.legacy_bios_bootable);
		...
		lbaint_t lastlba)
{
	...
	//@ assert \initialized(&gpt_h->signature);
	...
	//@ assert \initialized(&crc32_backup);
	 ...
	 */
	//@ assert  \initialized(&gpt_h->my_lba);
	 ...
	 * within the disk.
	 */
	  	//@ assert  \initialized(&gpt_h->first_usable_lba);
	if (le64_to_cpu(gpt_h->first_usable_lba) > lastlba) {
		printf("GPT: first_usable_lba incorrect: %llX > " LBAF "\n",
		       le64_to_cpu(gpt_h->first_usable_lba), lastlba);
		return -1;
	}
	  //@ assert  \initialized(&gpt_h->last_usable_lba);	  	  	
...
static int validate_gpt_entries(gpt_header *gpt_h, gpt_entry *gpt_e)
{
	...
	//@ assert \initialized(&gpt_h->partition_entry_array_crc32);
...
#define ___FRAMAC_align_buffer_spl_PATCH
  ALLOC_CACHE_ALIGN_BUFFER_PAD(gpt_header, gpt_head, 1, dev_desc->blksz);
	...
	//@ assert \initialized(&gpt_head->num_partition_entries);
	...
	//@  assert \initialized(&(gpt_pte+(part-1))->starting_lba);
...
static int pmbr_part_valid(struct partition *part)
{
	//@ assert \initialized(&part->sys_ind);  
...
{
	int i = 0;
	//@ assert \initialized(&mbr->signature);	
					 ...
					 gpt_header * pgpt_head)
{
	...
	//@ assert \initialized(&pgpt_head->num_partition_entries);
	//@ assert \initialized(&pgpt_head->sizeof_partition_entry);	
	...
	//@ assert \initialized(&pgpt_head->partition_entry_lba);
